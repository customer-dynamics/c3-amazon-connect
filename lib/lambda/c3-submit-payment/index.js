...
export async function handler(event) {
	...
	let paymentRequest = {};
	try {
		paymentRequest = await getPaymentRequest(contactAttributes.PaymentRequestId);
		console.log('Payment request response:', paymentRequest);
	} catch (error) {
		console.error(`Failed to get payment request for ${contactAttributes.PaymentRequestId}:`, error);
		throw error;
	}
	...
	try {
		...
		console.error(`Failed to post credit transaction for ${contactAttributes.PaymentRequestId}:`, error);
		throw error;
	}
}
