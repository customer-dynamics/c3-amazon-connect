...
export async function handler(event) {
	...
	try {
		...
		let paymentInfo = {
			name: `${contactAttributes.C3ContactFirstName} ${contactAttributes.C3ContactLastName}`,
			paymentAmount: +contactAttributes.C3PaymentAmount,
		...
		response = await postTransaction(
			contactAttributes.C3PaymentRequestId,
			paymentInfo,
		);
	} catch (error) {
