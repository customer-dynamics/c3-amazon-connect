...
const ZIFT_URL = process.env.C3_ENV === 'prod' ? 'https://secure.zift.io/gates/json?' : 'https://sandbox-secure.zift.io/gates/json?';
...
async function getAccountSecrets() {
	...
	if (!secrets.ZIFT_ACCOUNT_ID || secrets.ZIFT_ACCOUNT_ID === '<Your Zift account ID>') {
		throw new Error('Secret for ZIFT_ACCOUNT_ID is not set.');
	}
	if (!secrets.ZIFT_USERNAME || secrets.ZIFT_USERNAME === '<Your Zift username>') {
		throw new Error('Secret for ZIFT_USERNAME is not set.');
	}
	if (!secrets.ZIFT_PASSWORD || secrets.ZIFT_PASSWORD === '<Your Zift password>') {
