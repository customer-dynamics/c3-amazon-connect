...
export class Zift {
	...
	async tokenizeCard(cardNumber, expirationDate) {
		...
		const token = await this.#tokenize(cardNumber, expirationDate);
	...
	async tokenizeBankAccount(accountNumber, routingNumber) {
		...
		const token = await this.#tokenize(accountNumber, routingNumber);
	 ...
	 * @returns {Promise<string>} The payment token.
	 */
	async #tokenize(accountNumber, accountAccessory) {
		const accountSecrets = await this.#getAccountSecrets();
