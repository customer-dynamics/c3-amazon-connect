...
export class Zift {
	...
	async #tokenize(accountNumber, accountAccessory, paymentMethod) {
		const ziftCredentials = await this.#getZiftCredentials();
	 ...
	 *
	 * @returns {Promise<{accountId: string, username: string, password: string}>} The Zift account information.
	 */
	async #getZiftCredentials() {
		console.log('Getting secrets...');
		const secrets = await getSecrets('ziftCredentials');
		const ziftCredentials = secrets.ZIFT_CREDENTIALS;
		if (
