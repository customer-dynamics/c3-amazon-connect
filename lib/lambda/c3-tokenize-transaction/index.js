...
export async function handler(event) {
	...
	const encryptedCardNumber = event.Details.ContactData.Attributes.EncryptedCardNumber;
	let expirationDate = event.Details.ContactData.Attributes.ExpirationDate;
	const encryptedAccountNumber = event.Details.ContactData.Attributes.EncryptedAccountNumber;
	const routingNumber = event.Details.ContactData.Attributes.RoutingNumber;

	let paymentMethod;
	if (encryptedCardNumber && encryptedAccountNumber !== REDACTED || expirationDate && expirationDate !== REDACTED) {
		paymentMethod = 'Card';
		...
		console.debug('padded expiration date:', expirationDate);
	} else if (encryptedAccountNumber && encryptedAccountNumber !== REDACTED || routingNumber && routingNumber !== REDACTED) {
		paymentMethod = 'Bank';
	} else {
		console.error('Incomplete payment information found in contact attributes.');
		throw new Error('Incomplete payment information found in contact attributes.');
	}
	...
	const encryptedData = paymentMethod === 'Card' ? encryptedCardNumber : encryptedAccountNumber;
	let decryptedData;
	try {
	...
	try {
		...
		if (paymentMethod === 'Card') {
			paymentToken = await paymentGateway.tokenizeCard(decryptedData, expirationDate);
		} else {
			paymentToken = await paymentGateway.tokenizeBankAccount(decryptedData, routingNumber);
		}
