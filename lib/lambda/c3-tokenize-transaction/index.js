...
export async function handler(event) {
	...
	if (!keyId) {
		...
		return {
			statusCode: 400,
			error: 'CONNECT_SECURITY_KEY_ID is not set. This is the key ID From Amazon Connect.',
		};
	}

	// Get the encrypted credit card.
	console.log('Getting encrypted credit card...');
	const encryptedCard = event.Details.ContactData.Attributes.EncryptedCreditCard;
	if (!encryptedCard || encryptedCard.length < 1) {
		console.error('EncryptedCreditCard was not found in contact attributes.');
		return {
			statusCode: 400,
			error: 'EncryptedCreditCard was not found in contact attributes.',
		};
	}

	// Get the encrypted expiration date.
	console.log('Getting encrypted expiration date...');
	const encryptedExpiration = event.Details.ContactData.Attributes.EncryptedExpirationDate;
	if (!encryptedExpiration || encryptedExpiration.length < 1) {
		console.error('EncryptedExpirationDate was not found in contact attributes.');
		return {
			statusCode: 400,
			error: 'EncryptedExpirationDate was not found in contact attributes.',
		};
	}
	...
	if (!privateKey || privateKey === '<The content of your private key>') {
		...
		return {
			statusCode: 500,
			error: 'Secret for C3_PRIVATE_KEY is not set.',
		};
	}
	...
	try {
		...
		return {
			statusCode: 500,
			error: 'Failed to decrypt credit card.',
		};
	}
	...
	try {
		...
		console.error('Failed to decrypt expiration date:', expirationDecryptionError);
		return {
			statusCode: 500,
			error: 'Failed to decrypt expiration date.',
		};
	}
	...
	switch (process.env.C3_PAYMENT_GATEWAY) {
		case 'Zift':
			...
			if (paymentToken.error) {
				return {
					error: paymentToken.error,
				};
			}
			break;
		default:
			return {
				error: `Unable to tokenize payment for unknown gateway: ${process.env.C3_PAYMENT_GATEWAY}`,
			};
	}
	return {
