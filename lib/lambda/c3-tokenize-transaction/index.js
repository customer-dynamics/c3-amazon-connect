...
const PARAM_PRIVATE_KEY = 'C3_CONNECT_INPUT_DECRYPTION_KEY';
const PARAM_KEY_ID = 'C3_CONNECT_INPUT_KEY_ID';
...
export async function handler(event) {
	...
	const keyValues = await getSecrets(PARAM_PRIVATE_KEY, PARAM_KEY_ID);
	const privateKey = keyValues[PARAM_PRIVATE_KEY];
	if (!privateKey) {
		return {
			statusCode: 500,
	...
	const keyId = keyValues[PARAM_KEY_ID]; // This is the KeyID From Amazon Connect
	if (!keyId) {
